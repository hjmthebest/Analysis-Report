...
enuri = spark.sql(f"""
    ...
    with enuri_standard as (
        ...
        where 1 = 1
            ...
            and t1.lmt_st_dt between date_format(current_date(), 'yyyy-01-01') and date_format(current_date(), 'yyyy-12-31')   -- 에누리 시작일자: %Y-01-01 ~ %Y-12-31로 설정
        group by t1.dpstr_cust_no
           ...
           select  t5.cstr_cd
            ...
            where 1 = 1
                and t5.trns_dt between date_format(current_date(), 'yyyy-01-01') and date_format(current_date(), 'yyyy-12-31') -- 거래일자 범위: 2023-01-01 ~ 2023-12-31
	    ...
	    where 1 = 1
                and t6.trns_dt between date_format(current_date(), 'yyyy-01-01') and date_format(current_date(), 'yyyy-12-31') -- 거래취소일자 범위: 설정년도
